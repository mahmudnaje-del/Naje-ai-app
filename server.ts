...
async function getModelEndpointConfig(endpointId: string, defaultFallback?: string, _token?: string): Promise<{ modelId: string; fallbackModelId?: string; maxOutputTokens?: number; isEnabled?: boolean; supportedDurations?: number[] }> {
  ...
  try {
    ...
    if (doc.exists && doc.data()) {
      ...
      const fallbackModelId = data.fallbackModelId ? String(data.fallbackModelId).trim() : undefined;
...
async function generateWithFallback(opts: {
  ...
  let primaryModel = opts.defaultModelId || 'gemini-3.7-flash';
  ...
  if (!fallbackModel && opts.endpointId) {
    fallbackModel = FALLBACK_DEFAULTS[opts.endpointId] || (primaryModel.includes('pro') ? 'gemini-3.7-flash' : 'gemini-3.5-flash-lite');
  }
...
async function seedModelEndpointsIfMissing() {
  ...
  try {
    const endpointsToMigrate = [
      'text_lite', 'text_core', 'text_max', 'ui_standard', 
      'ui_builder', 'document_engine', 'doc_standard', 
      'document_writer', 'slide_writer',
      'tier_lite', 'tier_core', 'tier_max',
      'image_fast', 'image_standard', 'image_hd', 'image_pro',
      'video_standard', 'video_veo_lite', 'video_omni',
      'voice_tts', 'voice_tts_standard'
    ];
...
Return ONLY the refined, perfected shot list text in English without markdown code fences or conversational preamble.`;
    ...
    const result = await generateContentWithFallback(ai, {
      model: 'gemini-3.7-flash', // Auditor tier
...
User's request: "${rawPrompt}"`;
  ...
  // Phase 2: Final Stage-5 Self-Critique / Anti-AI-Slop Audit (Auditor tier: gemini-3.7-flash)
...
async function auditDocChunk(ai: any, sectionTitle: string, sectionHtml: string, contextSummary: string, auditorModel: string = 'gemini-3.7-flash'): Promise<{ ok: boolean; refinedHtml?: string; reason?: string }> {
  try {
...
async function auditSlideChunk(ai: any, slideData: any, auditorModel: string = 'gemini-3.7-flash'): Promise<{ ok: boolean; refinedSlide?: any; reason?: string }> {
  try {
...
app.post("/api/creative-pro-chat", async (req, res) => {
  try {
    ...
    let modelToUse = "gemini-3.7-flash"; // Naje Core (default)
    if (proMode === "thinking") {
      modelToUse = "gemini-3.7-flash"; // Naje Max
...
إذا طلب منك المستخدم توليد مستند (عرض تقديمي PowerPoint، مستند Word، أو ملف PDF)، فاستخدم الأداة (Function Call) المتاحة لك generate_document بدلاً من الرد بنص عادي لتفعيل محرك المستندات تلقائياً.${systemInstructionContext}`;
      ...
      if (type === 'image') {
        ...
        if (model === 'nova' || model === 'pro') {
          imageModelName = await getModelEndpointId('image_hd', 'gemini-3-pro-image', token);   // Naje Imagen Pro (nova)
        } else if (model === 'lite') {
          imageModelName = await getModelEndpointId('image_fast', 'gemini-3.1-flash-lite-image', token);  // Naje Imagen Lite
        } else {
          imageModelName = await getModelEndpointId('image_standard', 'gemini-3.1-flash-image', token);   // Naje Imagen
        }
        ...
        // Fallback or standard Interaction API call
        if (!base64Data) {
  ...
  "fontSize": number
        ...
        if (model === 'veo') {
          ...
          try {
            ...
            // Poll the operation synchronously
            let done = false;
            let attempt = 0;
            const maxAttempts = 60; // Up to 5 minutes (5s interval * 60)
            while (!done && attempt < maxAttempts) {
              const updated = await ai.operations.getVideosOperation({ operation: op });
              if (updated.done) {
                done = true;
                ...
                // Fetch the video securely from URI
                let videoRes = await fetch(uri, {
                  headers: { 'x-goog-api-key': process.env.GEMINI_API_KEY! }
                });
                if (!videoRes.ok) {
...
${speaker2Name}: لا والله، الجو مش صافي وفي تراب.`
        ...
        try {
          ...
          // Retry with standard fallback speech config or fallback model
...
{"finalHtml":"وسوم HTML نظيفة مثل <p> و <ul> و <strong> بدون <html> أو <body> أو علامات markdown"}`
              ...
              try {
                ...
                // Chunk-level Audit pass (Auditor tier: gemini-3.7-flash)
