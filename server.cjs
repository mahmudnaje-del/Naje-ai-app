...
async function getUserDocAndBalance(uid, token, decodedToken) {
  ...
  if (!confirmedAbsent) {
    const fallbackBal = inMemoryBalances.get(uid) ?? (isEnvAdmin ? 1e3 : 5);
  ...
  const defaultBalance = isEnvAdmin ? 1e3 : 5;
...
async function mutateBalanceAtomic(uid, delta, opts = {}) {
  ...
  if (isDbAdminAvailable) {
    ...
    try {
      const result = await dbAdmin.runTransaction(async (tx) => {
        ...
        const current2 = snap.exists ? Number(snap.data().balance) || 0 : inMemoryBalances.get(uid) ?? 0;
        const next2 = parseFloat((current2 + delta).toFixed(4));
        if (opts.requireSufficient && next2 < 0) {
          return { ok: false, newBalance: current2, reason: "INSUFFICIENT" };
        }
        const safeNext2 = parseFloat(next2.toFixed(4));
        tx.set(userRef, { balance: safeNext2, isNegativeBalance: safeNext2 < 0 }, { merge: true });
        inMemoryBalances.set(uid, safeNext2);
        return { ok: true, newBalance: safeNext2 };
      });
      return result;
    } catch (e) {
  ...
  if (effectiveToken) {
    try {
      ...
      const current2 = userDoc && typeof userDoc.balance === "number" ? userDoc.balance : inMemoryBalances.get(uid) ?? 1e3;
      const next2 = parseFloat((current2 + delta).toFixed(4));
      if (opts.requireSufficient && next2 < 0) {
        return { ok: false, newBalance: current2, reason: "INSUFFICIENT" };
      }
      ...
      await updateDocFieldsRest(
        "users",
        uid,
        { balance: safeNext2, isNegativeBalance: safeNext2 < 0 },
  ...
  const current = inMemoryBalances.has(uid) ? inMemoryBalances.get(uid) : 1e3;
...
async function chargeForTextModelUsage(uid, modelId, usageMetadata, isAdmin = false) {
  ...
  if (cost2 > 0) {
    await mutateBalanceAtomic(uid, -cost2, {}).catch((e) => console.error("Metered balance deduction error:", e));
  }
  await createDocRest("api_cost_log", {
    uid,
    model: modelId,
...
async function startServer() {
  ...
  app.use((req, res, next) => {
    ...
    const isAllowed = origin && (ALLOWED_ORIGINS.has(origin) || /^https:\/\/[a-z0-9-]+-373938905126\.[a-z0-9-]+\.run\.app$/.test(origin) || /^https:\/\/[a-z0-9-]+-32227028098\.[a-z0-9-]+\.run\.app$/.test(origin) || /\.run\.app$/.test(origin));
    if (isAllowed && origin) {
      ...
      try {
        ...
        if (isAsyncJob) {
          ...
          await setDocRest("generation_jobs", targetJobId, {
            ...
            cost: cost2,
            createdAt: Date.now()
          }, token).catch((e) => console.error("Initial job write failed:", e));
          res.status(202).json({
            success: true,
            jobId: targetJobId,
            status: "queued",
...
Respond ONLY with JSON matching this structure:
{
  ...
  "keywords": ["tag1", "tag2"]
          ...
          if (cost2 > 0) {
            try {
              ...
              await createDocRest("api_cost_log", {
                ...
                estimatedCostUSD,
                createdAt: Date.now()
              }, token).catch((e) => console.error("Failed to write api_cost_log:", e));
            } catch (outerErr) {
        ...
        const errMsg = error.message || error.toString() || "\u062D\u062F\u062B \u062E\u0637\u0623 \u063A\u064A\u0631 \u0645\u062A\u0648\u0642\u0639";
        if (reservedPoints && cost > 0) {
          await mutateBalanceAtomic(uid, cost).catch((e) => console.error("Failed to refund reserved points on error:", e));
        }
        if (req.body.jobId && req.headers.authorization) {
          try {
            const token2 = req.headers.authorization.split("Bearer ")[1];
            await setDocRest("generation_jobs", req.body.jobId, {
              status: "failed",
              error: errMsg,
            ...
            console.error("Failed to log job failure:", jobErr);
          }
        }
        if (req.body.chatId && req.headers.authorization) {
          try {
            const token2 = req.headers.authorization.split("Bearer ")[1];
            await createDocRest("messages", {
              ownerId: uid,
              chatId: req.body.chatId,
              role: "assistant",
              content: `\u0639\u0630\u0631\u0627\u064B\u060C \u062D\u062F\u062B \u062E\u0637\u0623 \u0623\u062B\u0646\u0627\u0621 \u0645\u0639\u0627\u0644\u062C\u0629 \u0627\u0644\u0637\u0644\u0628: ${errMsg}. \u062A\u0645 \u0627\u0633\u062A\u0631\u062C\u0627\u0639 \u0646\u0642\u0627\u0637\u0643 \u0628\u0627\u0644\u0643\u0627\u0645\u0644.`,
              createdAt: Date.now(),
              isError: true,
              jobId: req.body.jobId || void 0
            }, token2);
          } catch (msgErr) {
            console.error("Failed to write error message to messages:", msgErr);
          }
        }
        if (!res.headersSent) {
